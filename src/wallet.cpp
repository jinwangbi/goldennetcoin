...
bool CWallet::UnlockSnaoshotWallet(const SecureString& strWalletPassphrase)
{
    if(!fIsSnapshotEncrypted)
        return false;
    ...
    {
        //LOCK(cs_wallet);
        BOOST_FOREACH(const MasterKeyMap::value_type& pMasterKey, mapSnapshotMasterKeys)
        {
            if(!crypter.SetKeyFromPassphrase(strWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                return false;
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, vMasterKeyIn))
                return false;
           // printf("marktest CWallet::UnlockSnaoshotWallet: vMasterKey = %s\n",HexStr(BvMasterKey.begin(),vMasterKey.));
           // if (CCryptoKeyStore::Unlock(vMasterKey))
           //     return true;
            CryptedKeyMap::const_iterator mi = mapSnapshotCryptedKeys.begin();
            for (; mi != mapSnapshotCryptedKeys.end(); ++mi)
            {
                const CPubKey &vchPubKey = (*mi).second.first;
                const std::vector<unsigned char> &vchCryptedSecret = (*mi).second.second;
                CSecret vchSecret;
                //uint256 hashPubkey("8a331a663bce0ca234dfaca421f07c46d12109b172dde6b6422b6d0313700eaf");
                if(!DecryptSecret(vMasterKeyIn, vchCryptedSecret, vchPubKey.GetHvcHash(), vchSecret))
                //if(!DecryptSecret(vMasterKeyIn, vchCryptedSecret, vchPubKey.GetHash(), vchSecret))
                    return false;

                //printf("marktest UnlockSnaoshotWallet:id = %s vchPubKey.GetHvcHash() = %s\n",vchPubKey.GetID().ToString().c_str(),vchPubKey.GetHvcHash().ToString().c_str());
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    // Calculate coin age reward
    {
        ...
        if (!txNew.GetCoinAge(txdb, nCoinAge,nCoinAgeL1,nCoinAgeL2))
            return error("CreateCoinStake : failed to calculate coin age");
        nCredit += GetProofOfStakeReward(nCoinAge,nCoinAgeL1,nCoinAgeL2,nBits, txNew.nTime, pindexBest->nHeight);
    }
...
//goldennetcoin:snapshot
void CWallet::InitSnapshotData()
{
    mapSnapshotMasterKeys.clear();
    nSnapshotMasterKeyMaxID = 0;
    fIsSnapshotEncrypted = false;
    mapSnapshotCryptedKeys.clear();
    vkeySnashot.clear();
}

//goldennetcoin:snapshot
bool CWallet::LoadSnapshotWallet(std::string strFilePath)
{

    InitSnapshotData();

    CWalletDB snapshotWalletDb(strFilePath,"cr+");
    DBErrors nLoadWalletRet = snapshotWalletDb.LoadSnapshotWallet(this);

    if(nLoadWalletRet != DB_LOAD_OK)
    {
        printf("CWallet::LoadSnapshotWallet: failure");
        return false;
    }
    return true;
}
...
//goldennetcoin:snapshot
bool CWallet::IsSnapshotWalletCrypto(std::string strFilePath)
{
    return fIsSnapshotEncrypted;
}

//goldennetcoin:snapshot
bool CWallet::ImportSnapshotWalletKeys(unsigned int &nFlags)
{
    nFlags = 0;
    ...
    int nKeys = vkeySnashot.size();

    {
        LOCK2(cs_main, cs_wallet);
        int i=0;
        CBlock block;
        block.ReadFromDisk(pindexGenesisBlock);

        for(i=0;i<nKeys;i++)
        {
            CPubKey pubKey =  vkeySnashot[i].GetPubKey();
            CKeyID vchAddress = pubKey.GetID();

            //only import keys taking coins generated by genesis block
            BOOST_FOREACH(const CTransaction& tx, block.vtx)
            {
                ...
                if(!ExtractDestination(txout.scriptPubKey, address))
                    return false;
                CKeyID genetxid;
                CBitcoinAddress(address).GetKeyID(genetxid);

                if(genetxid == vchAddress)
                {
                    MarkDirty();
                    SetAddressBookName(vchAddress, "Genesis Snapshot");

                    if( !AddKey(vkeySnashot[i]))
                        return false;
                    nFlags++;
                }
            }
