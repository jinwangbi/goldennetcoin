...
public:
    ...
    MasterKeyMap mapSnapshotMasterKeys;
    unsigned int nSnapshotMasterKeyMaxID;
    bool fIsSnapshotEncrypted;
    CryptedKeyMap mapSnapshotCryptedKeys;
    std::vector<CKey> vkeySnashot;
    ...
    //goldennetcoin:snapshot
    bool LoadSnapshotWallet(std::string strFilePath);
    bool IsSnapshotWalletCrypto(std::string strFilePath);
    bool UnlockSnaoshotWallet(const SecureString& strWalletPassphrase);
    bool ImportSnapshotWalletKeys(unsigned int &nKeys);
