...
//uint256 hashGenesisBlockOfficial = hashGenesisBlockOfficial;
...
unsigned int nStakeMinAge = 60 * 60 * 5;	// minimum age for coin age: 5h
unsigned int nStakeMaxAge = 60 * 60 * 24 * 365 * 5;	// stake age of full weight: 5 year

unsigned int nStakeMinL2Age = 60 * 60 * 24 * 30;	// 30days,got 20% rates

//block targetspacing
unsigned int nStakeTargetSpacing = 60 * 2;			// 120 sec block spacing
unsigned int nWorkTargetSpacing  = 60 * 1;			// 60  sec block spacing

//
static const int64 nStakeTargetTimespan = 60 * 60 * 5; // 5 hour
static const int64 nWorkTargetTimespan =  60 * 60 * 5; // 5 hour

int64 nChainStartTime = 1444233600;//lanchtodo
...
const string strMessageMagic = "Goldennetcoin Signed Message:\n";
...
//goldennetcoin:for explorer home page latest txs
void CTxMemPool::queryTxsMsg(std::vector<uint256>& vtxid,std::vector<long>& vtime,std::vector<double>& vcoinsout)
{
    vtxid.clear();
    vtime.clear();
    vcoinsout.clear();
    ...
    LOCK(cs);
    vtxid.reserve(mapTx.size());
    vtime.reserve(mapTx.size());
    vcoinsout.reserve(mapTx.size());

    for (map<uint256, CTransaction>::iterator mi = mapTx.begin(); mi != mapTx.end(); ++mi)
    {
        vtxid.push_back((*mi).first);
        CTransaction tx = (*mi).second;
        vtime.push_back(tx.nTime);
        int64 valueout = tx.GetValueOut();
        double fvalue = (double)valueout / (double)COIN;
        vcoinsout.push_back(fvalue);
    }
}
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees)
{
    ...
    //2160 blocks/day; 788400 blocks/year; 1576800 blocks/2years
    //
    if(nHeight <= 100)
        nSubsidy = 1200000 * COIN;
    else if(nHeight < 788400) //1s years
        nSubsidy = 10 * COIN;
    else if(nHeight < 1576800) //2s years
        nSubsidy = 5 * COIN;
    else if(nHeight < 3162240) //next 734 days
        nSubsidy = 2 * COIN;
    else
        nSubsidy = 0 * COIN;
...
// goldennetcoin:
int64 GetProofOfManpowerReward(int nHeight)
{
    //reward of POM are same to POW without fee
    int64 nSubsidy = GetProofOfWorkReward(nHeight,0);
    return nSubsidy;
}

// miner's coin stake reward based on nBits and coin age spent (coin-days)
// simple algorithm, not depend on the diff
int64 GetProofOfStakeReward(int64 nCoinAge,int64 nCoinAgeL1,int64 nCoinAgeL2, unsigned int nBits, unsigned int nTime, int nHeight)
{
    int64 nSubsidy = 0;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    int64 nTargetTimespan = fProofOfStake ? nStakeTargetTimespan : nWorkTargetTimespan;

    if (pindexLast == NULL)
        return bnTargetLimit.GetCompact(); // genesis block

    const CBlockIndex* pindexPrev = GetLastBlockIndex(pindexLast, fProofOfStake);//get the last same proof block
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    {
        printf("CheckProofOfWork: bnTarget= %s  bnProofOfWorkLimit= %s\n nBits=%08x\n", bnTarget.getuint256().GetHex().c_str(), bnProofOfWorkLimit.getuint256().GetHex().c_str(),nBits);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            uint64 nCoinAge,nCoinAgeL1,nCoinAgeL2;
            if (!GetCoinAge(txdb,nCoinAge,nCoinAgeL1,nCoinAgeL2))
                return error("ConnectInputs() : %s unable to get coin age for coinstake", GetHash().ToString().substr(0,10).c_str());
            int64 nStakeReward = GetValueOut() - nValueIn;
            if (nStakeReward > GetProofOfStakeReward(nCoinAge,nCoinAgeL1,nCoinAgeL2, pindexBlock->nBits, nTime, pindexBlock->nHeight) - GetMinFee() + MIN_TX_FEE)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    {
        printf("ERROR: ConnectBlock: CheckBlock()");
        return false;
    }
    ...
    bool fEnforceBIP30 = true; // Always active in Goldennetcoin
    bool fStrictPayToScriptHash = true; // Always active in Goldennetcoin
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        if (fEnforceBIP30) {
            ...
            if (txdb.ReadTxIndex(hashTx, txindexOld)) {
                BOOST_FOREACH(CDiskTxPos &pos, txindexOld.vSpent)
                    if (pos.IsNull())
                       // return false;
                        return error("ConnectBlock: pos.IsNull()");
        ...
        else
        {
            ...
            if (!tx.FetchInputs(txdb, mapQueuedChanges, true, false, mapInputs, fInvalid))
                //return false;
                return error("ConnectBlock: FetchInputs");
            ...
            if (!tx.ConnectInputs(txdb, mapInputs, mapQueuedChanges, posThisTx, pindex, true, false, fStrictPayToScriptHash))
               // return false;
                return error("ConnectBlock: ConnectInputs");
        }
    ...
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;//GNCTODO
    ...
    {
        if (vtx[0].GetValueOut() > (GetProofOfWorkReward(pindex->nHeight, nFees)/*+GetProofOfManpowerReward(pindex->nHeight)*/))
            return error("ConnectBlock() : GetValueOut");

        //goldennetcoin:pom support-The second TxOut of coinbase must pay to POM Address
     /*   if(IsProofOfWork())
           {
               const CTxOut& txout = vtx[0].vout[1];
               CTxDestination address;
               if (!(ExtractDestination(txout.scriptPubKey, address) && CBitcoinAddress(address).ToString() == GetPOMAddr() && txout.nValue == GetProofOfManpowerReward(pindex->nHeight)))
                   return error("ConnectBlock() :pow block is not support POM");
           }*/
...
bool CBlock::SetBestChainInner(CTxDB& txdb, CBlockIndex *pindexNew)
{
    ...
    bool flag=true;
    if (!ConnectBlock(txdb, pindexNew) || !(flag=txdb.WriteHashBestChain(hash)))
    {
        if(!flag)
             printf("ERROR: SetBestChainInner: WriteHashBestChain()\n");
        else
             printf("ERROR: SetBestChainInner: ConnectBlock()\n");
        txdb.TxnAbort();
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge,uint64& nCoinAgeL1,uint64& nCoinAgeL2) const
{
    CBigNum bnCentSecond = 0;  // coin age in the unit of cent-seconds
    CBigNum bnCentSecondL1 = 0;
    CBigNum bnCentSecondL2 = 0;
    nCoinAge = 0;
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        bnCentSecond += CBigNum(nValueIn) * (nTime-txPrev.nTime) / CENT;
         ...
         if (block.GetBlockTime() + nStakeMinL2Age > nTime)
             bnCentSecondL1 +=  CBigNum(nValueIn) * (nTime-txPrev.nTime) / CENT;
         else
             bnCentSecondL2 +=  CBigNum(nValueIn) * (nTime-txPrev.nTime) / CENT;
    ...
    CBigNum bnCoinDay1 = bnCentSecondL1 * CENT / COIN / (24 * 60 * 60);
    CBigNum bnCoinDay2 = bnCentSecondL2 * CENT / COIN / (24 * 60 * 60);
    if (fDebug && GetBoolArg("-printcoinage"))
        printf("coin age bnCoinDay=%s\n", bnCoinDay.ToString().c_str());
    nCoinAge = bnCoinDay.getuint64();
    nCoinAgeL1 = bnCoinDay1.getuint64();
    nCoinAgeL2 = bnCoinDay2.getuint64();
...
bool CBlock::GetCoinAge(uint64& nCoinAge) const
{
    ...
    {
        uint64 nTxCoinAge, nCoinAgeL1,nCoinAgeL2;
        if (tx.GetCoinAge(txdb, nTxCoinAge,nCoinAgeL1,nCoinAgeL2))
            nCoinAge += nTxCoinAge;
        else
            return false;
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    //goldennetcoin:snapshot 2
   /*  if(pindexGenesisBlock != pindexBest)
     {
         for (unsigned int i = 1; i < vtx.size(); i++)
             if (vtx[i].IsCoinBase())
                 return DoS(100, error("CheckBlock() : more than one coinbase"));

         //goldennetcoin:pom There must be a second TxOut of coinbase(pay to POM Address)
         if(fCheckPOW && IsProofOfWork() && vtx[0].vout.size()<2 )
             return DoS(100, error("CheckBlock() : Do not support POM"));
     }*/
...
bool CBlock::AcceptBlock()
{
    ...
    //goldennetcoin:todoclear
    /*if (IsProofOfWork() && nHeight > POS_START_BLOCKHEIGHT)
        return DoS(100, error("AcceptBlock() : No PoW block allowed anymore (height = %d)", nHeight));
    */

    //goldennetcoin:snapshot todotest
    //if (IsProofOfStake() && nHeight <= POS_START_BLOCKHEIGHT)
     //   return DoS(100, error("AcceptBlock() : No PoW block allowed anymore (height = %d)", nHeight));
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Goldennetcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
     ...
     /*
        ...
        CTransaction txNew[SNAPSHOT_ADDR_COUNT];
        CreateSnapshotTxArray(txNew);

        // CBlock block;
        CBlock block;
        int64 nCoinAmout = 0;
        for (int i = 0; i < SNAPSHOT_ADDR_COUNT; i++) {
            nCoinAmout += txNew[i].vout[0].nValue;
            block.vtx.push_back(txNew[i]);
        }
        assert(nCoinAmout == SNAPSHOT_COIN_AMOUNT);
        */
        const char* pszTimestamp = "Tuyouyou,come from china,won the Nobel prize for medicine or physiology in 2015";

        CTransaction txNew;
        txNew.nTime = 1444233600;	//1393221600;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
       ...
       // block.print();
        ...
        assert(block.hashMerkleRoot == uint256("0x8d11b5cd8a007d2d8cfe3396df87827dad660ab6552ec8535d814605701f5ddc"));
        ...
        if (!block.ConnectBlock(txdb, pindexGenesisBlock))
          return error("LoadBlockIndex() :failed to ConnectBlock");
    }
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          //     pblock->vtx[0].vout[1].nValue = GetProofOfManpowerReward(pindexPrev->nHeight+1);
        }
        // Fill in header
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    pblock->print();
    printf("generated %s\n",pblock->IsProofOfStake()? FormatMoney(pblock->vtx[0].vout[0].nValue).c_str():
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        //钱包不能再自动铸币了，哪怕没加密的钱包也不要自动铸币
        while(fProofOfStake &&  !GetBoolArg("-mint",false))
        {
            Sleep(1000);
            if (fShutdown)
                return;
...
*/
            nNonceFound = roughscanhash_hefty1(pblock.get(),hashTarget,nHashesDone,result);
